...
for i in range(len(reviews_scraped_df['reviewCleaned'])):
    ...
    if(np.argmax(sentiment) == 1):
        positive_reviews = positive_reviews.append({'reviewText' : reviews_scraped_df['reviewText'][i], 'reviewCleaned' : reviews_scraped_df['reviewCleaned'][i]}, ignore_index = True)
    elif (np.argmax(sentiment) == 0):
        negative_reviews = negative_reviews.append({'reviewText' : reviews_scraped_df['reviewText'][i], 'reviewCleaned' : reviews_scraped_df['reviewCleaned'][i]}, ignore_index = True)
...
def clustering(corpus_for_clustering):
    ...
    while len(corpus_for_clustering) > 2:
        ...
        token_len = token_len + len(sent_tokenize(df_cluster['text'][0])) + len(sent_tokenize(df_cluster['text'][1]))
        ...
        for k in range(2, len(df_cluster)):
            if(len(sent_tokenize(corpus_for_clustering[k])) + token_len < 512):
                token_len = token_len + len(sent_tokenize(corpus_for_clustering[k]))
...
def final_steps(content_list):
    ...
    while (len(content_list) > 1):
        ...
        final_summary = summarizer(wre_clusters)
        content_list = final_summary
    return final_summary

positive_summary = final_steps(positive_reviews['reviewText'].tolist())
